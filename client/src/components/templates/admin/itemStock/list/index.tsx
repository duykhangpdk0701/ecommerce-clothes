...
import IItemStock from "@/interfaces/ItemStock";
import ListItemStockToolbar from "./section/ListToolbar";
import ListItemStockHead from "./section/ListHead";
...
function getComparator(order: "asc" | "desc", orderBy: OrderByType) {
  return order === "desc"
    ? (a: IItemStock, b: IItemStock) => descendingComparator(a, b, orderBy)
    : (a: IItemStock, b: IItemStock) => -descendingComparator(a, b, orderBy);
}
...
function applySortFilter(
  ...
  if (query) {
    return filter(
      array,
      (_user) => _user.id.toString().indexOf(query.toLowerCase()) !== -1
    );
  }
  return stabilizedThis.map((el) => el.el);
}

interface IListItemStockTempalte {
  data?: IItemStock[];
}

const ListItemStockTemplate: FC<IListItemStockTempalte> = (props) => {
  const { data } = props;
...
export default ListItemStockTemplate;
